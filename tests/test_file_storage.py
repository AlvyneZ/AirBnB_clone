...
class TestBaseModel(unittest.TestCase):
    ...
    Methods:
        ...
        test_save_to_file: tests the save_to_file method of BaseModel class
        test_load_from_file: tests the load_from_file method of BaseModel class
    """

    def tearDown(self):
        """
        Runs at the end of each test to reset BaseModel identifier count
        """
    ...
    def test_init(self):
        ...
        self.assertEqual(
            ...
            "initializer should set id with the provided value")
        self.assertEqual(
            BaseModel._BaseModel__nb_objects, 4,
            "Recorded number of objects should equal objects "
            "initialized with id not set")

    def test_to_json_string(self):
        """
        Test for to_json_string method of BaseModel class
        """
        self.assertEqual(
            BaseModel.to_json_string(None),
            "[]",
            "converter should return a JSON of an empty list for None arg"
        )
        self.assertEqual(
            BaseModel.to_json_string([]),
            "[]",
            "converter should return a JSON of an empty list for [] arg"
        )
        self.assertEqual(
            BaseModel.to_json_string([{"a": 1, "b": 2}]),
            """[{"a": 1, "b": 2}]"""
        )
        self.assertEqual(
            BaseModel.to_json_string([{"a": 1, "b": 2}, {"c": 3}]),
            """[{"a": 1, "b": 2}, {"c": 3}]"""
        )

    def test_from_json_string(self):
        """
        Test for from_json_string method of BaseModel class
        """
        self.assertListEqual(
            BaseModel.from_json_string(None),
            [],
            "converter should return an empty list for None arg"
        )
        self.assertListEqual(
            BaseModel.from_json_string("[]"),
            [],
            "converter should return an empty list for [] arg"
        )
        self.assertListEqual(
            BaseModel.from_json_string("""[{"a": 1, "b": 2}]"""),
            [{"a": 1, "b": 2}]
        )
        self.assertListEqual(
            BaseModel.from_json_string("""[{"a": 1, "b": 2}, {"c": 3}]"""),
            [{"a": 1, "b": 2}, {"c": 3}]
        )

    def test_create(self):
        """
        Test for create method of BaseModel class
        """
    ...
    def test_save_to_file(self):
        """
        Test for save_to_file method of BaseModel class
        """
    ...
    def test_load_from_file(self):
        """
        Test for load_from_file method of BaseModel class
        """
        files_existed = self.__backup_og_files()
        try:
            self.assertListEqual(
